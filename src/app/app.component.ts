...
type cachedData = { repos :repoType , repoShow : repoType,userData : userType, 
  profileName:string ,product:number,pageNo:number ,pageSize: number}
...
export class AppComponent {
  ...
  //DROPDOWN HANDLER
    ...
    this.userData={};
    try{
           const resUser = await lastValueFrom (this.apiService.getUser(this.profileName));
    ...
    if (!this.repos.hasOwnProperty(this.profileName) || !Array.isArray(this.repos[this.profileName])) {
      ...
      {
        this.repoShow[this.profileName]=currentrepo.slice(requiredFrom,requiredTill)
        this.display = 'none'
        this.storeDataInLS(pageNo ,pageSize )
       }

       else if(this.userData[this.profileName].totalRepos > this.repos[this.profileName].length){
      const resRepo = await lastValueFrom (this.apiService.getRepos(name,pageNo,pageSize))
      if (resRepo.length > 0) {
      const reducedArray: repotypeMap =
      await Promise.all( resRepo.map(async(ele:any) => {
        const repoLangs = await lastValueFrom (this.apiService.getLangs(this.profileName, ele.name));
        return    {
              repoName: ele.name,
    ...
    this.repos[this.profileName]=[...this.repos[this.profileName],...reducedArray];
